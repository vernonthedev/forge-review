...
export function createWebhookRoutes(environment: AppEnv) {
  ...
  webhookRoutes.post('/', async (c) => {
    ...
    try {
      ...
      if (!verifyWebhookSignature(payload, signature, environment.GITHUB_WEBHOOK_SECRET)) {
      ...
      const installationToken = await createInstallationToken(
        { appId: environment.GITHUB_APP_ID, privateKey: environment.GITHUB_PRIVATE_KEY },
        installationId
      );
      ...
      const providerConfig = {
        ...
        apiKey: mergedConfig.model.apiKey ?? (mergedConfig.model.baseUrl ? undefined : environment.NVIDIA_API_KEY),
